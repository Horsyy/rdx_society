...
function RefreshBussHUD()
	...
	if RDX.PlayerData.job.grade_name == 'boss' then
		EnableSocietyMoneyHUDElement()

		RDX.TriggerServerCallback('rdx_society:getSocietyMoney', function(money)
			UpdateSocietyMoneyHUDElement(money)
		end, RDX.PlayerData.job.name)
	end
end

RegisterNetEvent('rdx_addonaccount:setMoney')
AddEventHandler('rdx_addonaccount:setMoney', function(society, money)
	if RDX.PlayerData.job and RDX.PlayerData.job.grade_name == 'boss' and 'society_' .. RDX.PlayerData.job.name == society then
		UpdateSocietyMoneyHUDElement(money)
	end
end)
...
function EnableSocietyMoneyHUDElement()
	...
	if RDX.GetConfig().EnableHud then
		RDX.UI.HUD.RegisterElement('society_money', 3, 0, societyMoneyHUDElementTpl, {
			money = 0
		})
	end

	TriggerEvent('rdx_society:toggleSocietyHud', true)
end

function DisableSocietyMoneyHUDElement()
	if RDX.GetConfig().EnableHud then
		RDX.UI.HUD.RemoveElement('society_money')
	end

	TriggerEvent('rdx_society:toggleSocietyHud', false)
end

function UpdateSocietyMoneyHUDElement(money)
	if RDX.GetConfig().EnableHud then
		RDX.UI.HUD.UpdateElement('society_money', {
			money = RDX.Math.GroupDigits(money)
		})
	end

	TriggerEvent('rdx_society:setSocietyMoney', money)
end
...
function OpenBossMenu(society, close, options)
	...
	RDX.TriggerServerCallback('rdx_society:isBoss', function(isBoss)
		if isBoss then
			...
			RDX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
				title    = _U('boss_menu'),
				align    = 'left',
				elements = elements
			}, function(data, menu)
				if data.current.value == 'withdraw_society_money' then
					RDX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
				...
				elseif data.current.value == 'deposit_money' then
					RDX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_' .. society, {
				...
				elseif data.current.value == 'wash_money' then
					RDX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_' .. society, {
...
function OpenManageEmployeesMenu(society)
	RDX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees_' .. society, {
...
function OpenEmployeeList(society)
	RDX.TriggerServerCallback('rdx_society:getEmployees', function(employees)
		...
		RDX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
				RDX.ShowNotification(_U('you_have_fired', employee.name))

				RDX.TriggerServerCallback('rdx_society:setJob', function()
					OpenEmployeeList(society)
...
function OpenRecruitMenu(society)
	RDX.TriggerServerCallback('rdx_society:getOnlinePlayers', function(players)
		...
		RDX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. society, {
			title    = _U('recruiting'),
			align    = 'left',
			elements = elements
		}, function(data, menu)
			RDX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
				...
				if data2.current.value == 'yes' then
					RDX.ShowNotification(_U('you_have_hired', data.current.name))
...
function OpenPromoteMenu(society, employee)
	RDX.TriggerServerCallback('rdx_society:getJob', function(job)
		local elements = {}
		...
		RDX.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_employee_' .. society, {
			title    = _U('promote_employee', employee.name),
			align    = 'left',
			elements = elements
		}, function(data, menu)
			menu.close()
			RDX.ShowNotification(_U('you_have_promoted', employee.name, data.current.label))
...
function OpenManageGradesMenu(society)
	RDX.TriggerServerCallback('rdx_society:getJob', function(job)
		...
		for i=1, #job.grades, 1 do
			...
			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(gradeLabel, _U('money_generic', RDX.Math.GroupDigits(job.grades[i].salary))),
				value = job.grades[i].grade
			})
		end

		RDX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_grades_' .. society, {
			title    = _U('salary_management'),
			align    = 'left',
			elements = elements
		}, function(data, menu)
			RDX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_grades_amount_' .. society, {
				...
				elseif amount > Config.MaxSalary then
					RDX.ShowNotification(_U('invalid_amount_max'))
				else
					menu2.close()

					RDX.TriggerServerCallback('rdx_society:setJobSalary', function()
						OpenManageGradesMenu(society)
...
AddEventHandler('rdx_society:openBossMenu', function(society, close, options)
